...
function ProjectCard({ project, index, isCustom }: ProjectCardProps) {
  if (isCustom) {
    const customProject = project as CustomProject;
    return (
      <Card
        key={`custom-${customProject.id}`}
      ...
      >
        <CardBody className="p-4">
          ...
          <div className="space-y-2">
            {customProject.topics && customProject.topics.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {customProject.topics.slice(0, 6).map((topic) => (
                  <Chip
                    key={topic}
  ...
  const repo = project as Repository;
  return (
    <Card
      key={repo.id}
      isExternal
...
export function Projects({ refreshTrigger }: ProjectsProps) {
  ...
  const githubRepos = reposCache[sortBy] || [];
  const allProjects = [...customProjects, ...githubRepos];
  const hasGitHub = portfolioData?.social?.github;
  const hasCustomProjects = customProjects.length > 0;
  const hasAnyProjects = hasGitHub || hasCustomProjects;
