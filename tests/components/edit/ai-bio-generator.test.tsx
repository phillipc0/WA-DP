...
describe("AIBioGenerator", () => {
  ...
  it("renders the AI bio generator component", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Generate AI Bio" }),
    ).toBeInTheDocument();
  ...
  it("shows validation error when name is missing", async () => {
    ...
    const generateButton = screen.getByRole("button", {
      name: "Generate AI Bio",
    });
    fireEvent.click(generateButton);

    await waitFor(() => {
      expect(
        screen.getByText("Error: Name is required to generate a bio"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows validation error when title is missing", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "Error: Professional title is required to generate a bio",
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows validation error when skills array is empty", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "Error: At least one skill is required to generate a bio",
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  it("successfully generates bio and calls onBioGenerated", async () => {
    ...
    render(
      <AIBioGenerator {...defaultProps} onBioGenerated={onBioGenerated} />,
    );
  ...
  it("handles API error response without error message", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Error: Failed to generate bio"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("closes settings modal when modal onClose is called", () => {
    ...
    expect(
      screen.queryByText("Gemini API Configuration"),
